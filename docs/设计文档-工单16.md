...
> 编制日期：2026 年 9 月
> 版本：v1.4（2026-09-17 修订；阶段二数字人形象层落地 + 修正 v1.2 漏改残留）
>
...
> | **v1.4 阶段二落地（同日）** | ① 3.1 节末尾「阶段二扩展位（本期不实现）」改为**落地说明**：两个 provider 接口（形象驱动 / TTS）的实际实现、接口清单与职责边界，Lip Sync 明确**不作为独立接口**；② 6.4 交付物清单补数字人层两行（源码 + 证据，无对应工单号）；③ 6.5 阶段表标注阶段二已完成；④ **修正 v1.2 漏改残留**——6.4/6.5 共 6 处仍写「工单16~**20**」「17/18/19/**20**」，与工单 20 已移出的范围变更矛盾，统一改为 16~19 |
...
**阶段二扩展位（已落地，见下方 v1.4 说明）**：在"终端层"与"应用层"之间预留**数字人层**（Avatar / TTS / Lip Sync），通过 provider 接口与能力层解耦，接入选型为臻灵或讯飞虚拟人 API。

> **v1.4 落地说明（2026-09-17）**：阶段二已实现该层的**本地 provider**，「接入云端」仍留给阶段三。
> 按 `CLAUDE.md:19` 的粒度只开**两个**接口（形象驱动、TTS）；**Lip Sync 不作为独立接口**——它是形象驱动接口的输入，由音量信号驱动。
> - **形象驱动** `frontend/src/avatar/provider.js`：`pickAvatarProvider(AVATAR_PROVIDER)` → `{ name, palette, mouthPath(open), computePose({ volume, state, t, blink }) }`；本地实现 `svg-face`（纯代码内联 SVG，零素材依赖）。
> - **TTS** `backend/app/services/tts.py`：`synthesize(text, voice)` 按 `TTS_PROVIDER` 分派；本地实现 `edge`（Edge-TTS，纯 CPU、无 Key、需联网，结果落盘 `data/tts_cache/`）。接口 `GET /api/tts/config`、`GET /api/tts/voices`、`POST /api/tts/speak`（返回音频字节流，不包 `ApiResponse`，同 `kb.py` 回显图片的先例）。
> - **口型的驱动信号是真的**：前端 `AudioBufferSourceNode → AnalyserNode`，rAF 读 RMS 映射为张嘴幅度，由实测音量驱动，不是按固定节奏播放的假动画（浏览器取证已断言发声期间口型形状种类数与音量峰值）。
> - **职责边界**：前端只管「在哪切句」、后端只管「怎么念」。切句必须在前端（要低延迟，首句不等整篇写完），Markdown 清洗必须放后端（前端无测试框架，而这段最易出错）。
> - **挂载位置**：仅智能助教问答页 `Chat.vue` 右侧固定栏，不改 `backend/app/api/assistant.py` 的 SSE 契约——数字人层不得影响教学大脑。
>
> 阶段三接云端数字人 = 新增一个 provider 实现并改 `.env` 的 `AVATAR_PROVIDER` / `TTS_PROVIDER`，问答页与舞台代码不动。
...
| GET | `/api/learn/kp/aliases` | **全部别名列表（v1.4 补录）**，`?only_placeholder=`。教师抽屉「全部别名」页签的数据源，也是误挂改判的入口；`?only_placeholder=true` 时等价于未归类清单 | teacher |
...
| GET | `/api/learn/profile` | 当前学生画像（各知识点 `mastery`）。**权威来源为实时计算**：读 `attempts` ∪ `score_imports` 现算，过滤 `is_placeholder`；`student_profile` 表仅作缓存，**缓存缺失或过期时必须实时兜底重算，不得返回空画像**（见 3.3.3） | 登录用户 |
| GET | `/api/learn/path` | 学习路径推荐（含顺序与可解释理由；**过滤 `is_placeholder`**） | 登录用户 |
...
| GET | `/api/learn/exam` | **试卷模式·组卷（v1.3）**：`?plan_id=`（缺省取最近一次含试题的备课记录），按 `source='exam'` + **同源**（`exam_questions.plan_id` 相同）返回**整套题**（题干/选项/分值，**不含答案**）。题库无该套试题时返回明确空态，不得静默返回空列表 | 登录用户 |
...
| GET | `/api/learn/mistakes` | 错题本列表 | 登录用户 |
| GET | `/api/learn/mistakes/{id}` | **错题详情（v1.4 补录）**：单条错题的完整分析 + 变式题 | 登录用户 |
...
| GET | `/api/learn/related` | 错题详情页侧栏 / 仪表盘今日任务 / **学习路径页每个节点（v1.3）** 三处共用：按 `kp_id` 返回三样推荐（相关提问 / 关联学习资料 / 同知识点练习题）。**对该生做一次惰性补挂**：只扫该生尚无 `message_kp` 的消息并即时挂靠（量极小），避免"刚提问 → 当场看侧栏为空"必须依赖教师先跑批处理 sync。**不改工单18 的表与 API**。路径页节点入口对齐原文"沿学习路径学习，系统在旁推送相关微课、资料" | 登录用户 |

> **v1.4 权限口径澄清**：表中原先把 `/profile`、`/path`、`/exam`、`/mistakes`、`/related` 等**只读**接口标为 `student`，与"教师抽屉挂在 `/learn/path` 页内"（见下）**自相矛盾**——教师若被 403 挡在 `/learn/path` 外，抽屉永远打不开，`kp/sync` 与 `kp/merge` 也随之无人可及。故明确为：**只读接口 = 登录用户；作答类写接口（`/answer`、`/exam/submit`、错题分析、成绩导入）= student；图谱管理接口 = teacher**。写接口不得放宽到"登录用户"——画像与错题本是按 `user_id` 隔离的个人数据，教师没有代答的语义。
...
>
> **v1.4 补录 `GET /api/learn/mistakes/{id}` 的理由**：错题本页是"左列表 + 右详情"，而 3.2.7 定稿的分析结构（解析 + 归因 + 2~3 道变式题，含答案与解析）**单条就可能上千字**。若列表接口带上全文，响应体会随错题数线性膨胀——试点期几十条尚可，但这属于"为了少写一个接口而把列表做成详情"的将就。详情单独取，列表只返回题干、知识点、状态与时间。
...
| 入口 | 匹配方式 | 适用输入 | 调用方 |
| --- | --- | --- | --- |
| `match_label(text, *, index, on_unmatched, use_vector=False)` | ① 归一化后全等（全半角、空白、标点、大小写、常见后缀"算法/方法/技术"）→ ② 查 `kp_alias` 别名表 → ③ **图谱词典最长子串扫描**（v1.4 插入，见下）→ ④（可选）bge-m3 余弦 ≥ 0.85 | 短标签、单元格文本 | 题目同步、历史成绩导入 |
| `match_text(text, *, index, on_unmatched, use_vector=False)` | ① **图谱词典最长子串扫描**（`knowledge_points.name` ∪ `kp_alias.raw_label` 合成 40~200 条纯字符串词典）→ ② bge-m3 余弦兜底 | 整句口语化提问 | 助教问题挂靠 |

> **v1.4 实测修订：`match_label` 插入词典档（2026-09-17，seed 完成后用真实数据量化得出）**
> 原漏斗只有「全等 → 别名 → 向量」三档。在**已经生成的 92 道演示题**上跑实测，
> 未归类占比 **47.8%**——**远超本文档自己定的 20% 判据线**，按原文该"打开向量档"。
> 但根因不是语义距离，而是**标签形态**：老记录（prompt 修正前生成）的标签是
> 「反向传播的基本原理与适用范围」「学习率对训练的影响」这类**短语**，
> 而图谱节点是「反向传播」「学习率」这类**名词**。这类输入**本来就该走词典扫描**——
> `match_text` 早就有这一档，只是 `match_label` 没用上。
>
> | 漏斗 | 命中 | 未归类 |
> | --- | --- | --- |
> | ① 全等 + ② 别名（原设计） | 48/92 = 52.2% | **44/92 = 47.8%** |
> | 追加 ③ 词典扫描 | 82/92 = 89.1% | **10/92 = 10.9%** |
> | ③ 后再拆一个复合节点名（见下） | 87/92 = 94.6% | **5/92 = 5.4%** |
>
> 插入词典档后未归类降到 10.9%，**远低于 20% 判据线，向量档继续关闭**——
> 正合原文"优先补别名与后缀规则（更便宜也更准）"的指引。
> 词典档是确定性规则（最长子串 + 跨度占位，不重复挂靠），结果可解释、零 API 成本，
> 与向量档并不冲突：真出现"词典里没有同义词"的漂移时，向量档开关仍然留着。
>
> 余下 4 种未归类标签（"误差项的反向传递""误差项与梯度递推""输出层误差项的计算"
> "权重梯度的计算公式"）**刻意不再用规则吃掉**：它们全是「反向传播」的细节表述，
> 正好作为教师端「待归并清单」的演示素材——归并一次即落 `kp_alias`，下次自动命中。
> 另有 **3 种图谱节点命名反例**由实测暴露：节点若起成"梯度消失与梯度爆炸"这种**复合名**，
> 词典扫描反而匹配不上"梯度消失问题""梯度消失的缓解方法"等真实标签（实测少命中 4 道题），
> 已拆为「梯度消失」「梯度爆炸」两个独立节点。**这条约束适用于后续扩充图谱**：
> 节点名用单一概念，不要把两个知识点合成一个节点名。
>
> 同时澄清 `match_text` 的两处签名（原文字面有歧义）：候选集参数名统一为 `index`（承载
> `KpIndex{nodes, aliases, placeholder_id}`，因为别名表与占位节点都是匹配必需的只读输入，
> 分散成多个参数容易漏传）；`min_score` 改为模块常量 `VECTOR_THRESHOLD=0.85`。
...
**迁移脚本**：`backend/scripts/migrate_w19.py`，**幂等**（先查 `PRAGMA table_info` 再 `ALTER TABLE ADD COLUMN`），可重复执行。**结构变更不走 `create_all`**。

> **实现修订（2026-09-17，代码与本节原始表述的差异，以本节为准）**：迁移逻辑放在 `app/migrations.py`，
> `scripts/migrate_w19.py` 降为 CLI 入口，**并由 `app.db.init_db()` 在启动时自动调用**。两点理由：
> ① **不让 `app` 反向依赖 `scripts`**——`init_db()` 要调迁移，逻辑留在 scripts 里会形成包路径与依赖方向的别扭；
> ② **把"启动前运行一次"从人工纪律变成代码保证**。原表述要求"启动前运行一次"，漏跑的症状是
> `no such column: exercises.kp_id`，且只在查询时才炸——而 pytest 用临时库从零建表，
> **恰恰走不到 ALTER 分支**，漏跑在测试里也发现不了。自动化后这个失败模式不存在了。
> 迁移的 ALTER 分支改由两条专门用例覆盖（手工造"旧库"→ 跑迁移 → 断言补列 + 重跑幂等 + 老行可读），
> 因为正常测试路径永远走不到它。
...
> **执行记录（2026-09-17 完成）**：按上述顺序落地，三点实测结论存档：
> ① **孤儿清单为空**——对 6 组外键关系全量核对（`plan_versions`/`coursewares`/`exercises`/`exam_questions`→`teaching_plans`、`kb_chunks`→`kb_docs`、`messages`→`conversations`），**0 条孤儿**，无需清理，故本轮只做"补用例 + 开 pragma"两步。
> ② **用例有效性已反证**：临时把 pragma 改回 `OFF` 重跑，4 条用例**全部转红**（含"删文档后切块仍在"），确认它们拦的是真问题而非恒真断言；改回 `ON` 后全绿。
> ③ **回归无副作用**：全量 pytest 101 → **112 条全绿**，工单 17/18 的删除路径未受影响。
>
> 另一处顺带修正：`app/main.py` 的 FastAPI `description` 原写"工单16~20…面试AI复盘"，与 v1.2 的范围变更不符，已改为"工单16~19…（工单20 已移出本期）"。
...
| --- | --- | --- |
| 设计文档 | 本文档（六章） | 16 |
| 技术设计 | UML 流程图、接口设计、数据库设计（本文档第三章） | 17/18/19 |
| 源码 | `backend/` + `frontend/`（文件头注释含工单编号） | 17/18/19 |
| 测试 | `backend/tests/pytest_工单*.py` 及运行结果 | 17/18/19 |
| 验收证据 | `docs/evidence/工单XX/` 截图与录屏 | 16~19 |
| 数字人层源码 | `frontend/src/avatar/`、`frontend/src/audio/`、`backend/app/services/tts.py`（阶段二，无对应工单号） | 阶段二 |
| 数字人层证据 | `docs/evidence/阶段二/` 截图 + `capture_evidence.py` 的 avatar 场景断言 | 阶段二 |

### 6.5 后续演进方向

| 阶段 | 内容 | 前置条件 |
| --- | --- | --- |
| 阶段二 | 数字人形象层：前端 2D 虚拟形象 + TTS + 音量驱动口型；抽象为可替换 provider | **工单16~19 全部验收通过**（✅ 已于 2026-09-17 完成） |
| 阶段三 | 接入云端数字人 API（臻灵 / 讯飞虚拟人），仅替换 provider 实现 | 阶段二完成；商务采购落地 |
| 阶段四 | 实时全双工教学对话（<200ms、支持打断） | 阶段三完成 |
| 并行方向 | 管理侧智能体（教务管理、迎新、智能办公助手） | 本期不含 |

> **纪律要求**：阶段划分严格串行。阶段一（工单 16~19）全部完成并通过验收之前，不得开始数字人层开发，数字人层不得阻塞或影响工单交付。
>
> **v1.4 进度标注（2026-09-17）**：阶段一（工单 16~19）与阶段二（数字人形象层）均已完成。阶段二的落地细节见 3.1 节末尾的 v1.4 说明。阶段三需商务采购落地后方可启动。
