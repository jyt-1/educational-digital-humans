 ...
 *
 * 注意二次贝塞尔的**实际**视觉高度只有控制点偏移量的一半
 * （起点终点同高时，曲线中点 = 起点 + 控制点偏移/2）。
 * 所以 h 的系数要按「视觉高度 ≈ h/2」来定：系数 15 时全张也才 7px，
 * 在脸上几乎看不出张嘴，实测截图才发现，故取 30（全张视觉高度约 16px）。
 */
function mouthPath(open) {
  const h = 2 + open * 30 // 至少 2px，否则闭嘴时形状退化成不可见的零面积
